...
export function AnimeDetailPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {((data.anilistRelated ?? []).length > 0 || data.relatedAnime.length > 0) && (
        <section className="space-y-3">
          <h2 className="font-display text-2xl font-bold">المواسم والأعمال ذات الصلة</h2>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
            {/* AniList relations (primary — accurate, with type badges) */}
            {(data.anilistRelated ?? []).map((r) => (
              <Link
                key={r.href}
              ...
              >
                <div className="relative aspect-[2/3] overflow-hidden rounded-lg bg-surface">
                  ...
                  <span className="absolute start-2 top-2 rounded-md bg-accent/90 px-2 py-0.5 text-[10px] font-semibold text-white backdrop-blur-sm">
                    {r.type}
                  </span>
            ...
            {/* Scraper fallback (for items not covered by AniList) */}
            {data.relatedAnime
              .filter((r) => !(data.anilistRelated ?? []).some((a) => a.href === r.href))
              .map((r) => (
                <Link
                  key={r.href}
                  to={`/anime/${encodeURIComponent(r.href)}`}
